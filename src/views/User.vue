<template>
	<div>
		...
		<el-col>
			<el-button v-if="opers.insert" type="success" @click="adds()">添加</el-button>
			<el-button v-if="opers.delete" type="danger" @click="deletes()">批量删除</el-button>
		</el-col>
		...
		<el-table :data="tableData" style="width: 100%" @selection-change="handleSelectionChange">
			...
			<el-table-column label="操作" width="400px">
				<template slot-scope="scope">
					...
					<span v-if="loginname==scope.row.username">
						...
						<el-button size="mini" type="danger" @click="xiu(scope.row.id)">修改密码</el-button>
						<el-button size="mini" type="danger" @click="del(scope.row.id)">删除</el-button>
					</span>
					<!--全部存在-->
					<span v-else>
						<el-button size="mini" v-if="opers.update" type="primary" @click="find(scope.row.id)">编辑</el-button>
						<el-button size="mini" v-if="opers.detail" type="success" @click="xiang(scope.row.id)">详情</el-button>
						<el-button size="mini" v-if="opers.update" type="danger" @click="xiu(scope.row.id)">修改密码</el-button>
						<el-button size="mini" v-if="opers.delete" type="danger" @click="del(scope.row.id)">删除</el-button>
					</span>
...
<script>
	export default {
		...
		created: function() {
			this.init();
			//登录名称
			this.loginname = this.$store.state.user.username;
			//是路由route的 query参数对象
			...
			if (oper) {
				//判断有没有详情功能
				if (oper.indexOf('detail') != -1) {
					this.opers.detail = true;
				}
				//判断有没有删除功能
				if (oper.indexOf('delete') != -1) {
					this.opers.delete = true;
				}
				//判断有没有更新龚
				if (oper.indexOf('update') != -1) {
					this.opers.update = true;
				}
				//判断有没有添加功能
				if (oper.indexOf('insert') != -1) {
					this.opers.insert = true;
				}
			} else {
				//console.log("为空操作");
			}
		...
		methods: {
			del(id) {
				this.$confirm('确认删除此数据, 是否继续?', '提示', {
					...
					this.axios.post("http://127.0.0.1/user/dels", null, {
						params: {
							id: id
						}
					}).then((response) => {
			...
			xiu(id) {
				this.axios.get("http://127.0.0.1/user/find", {
					...
					this.passwords = response.data.password
					this.ruleForm.id = id
					this.xius = true
				})
			},
			...
			handleCheckedCitiesChange(value) {
				...
				console.log(ss)
				console.log(typeof(this.checkedCities))
			},
			roleupdate() {
			...
			deletes() {
				...
				// console.log(typeof(this.delets))
				if (this.delets != null && this.delets.length > 0) {
					this.del(this.delets.map(uid => uid.id).join(","))
				} else {
